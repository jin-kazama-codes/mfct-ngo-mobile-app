...
export default function ProfileScreen() {
  ...
  const roleName = (activeUser?.role || currentRole || 'Member')
    .replace(/_/g, ' ')
    .toUpperCase();
  ...
  return (
    <View className="flex-1 bg-slate-50 dark:bg-slate-950">
      ...
      >
        <Text className="text-xl font-bold text-slate-900 dark:text-white">My Profile</Text>
        <Text className="text-xs text-slate-500 dark:text-slate-400">Manage account & preferences</Text>
      </View>
      ...
      >
        ...
        <View className="bg-white dark:bg-slate-900 p-5 rounded-3xl mb-5 border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <View className="flex-row items-center gap-3 py-2.5 border-b border-slate-100 dark:border-slate-800">
            ...
            <View className="flex-1">
              ...
              <Text className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                {activeUser?.communityName || 'Bareilly Central Care Society (Headquarters)'}
              </Text>
            </View>
          </View>
        ...
        <View className="bg-white dark:bg-slate-900 p-5 rounded-3xl mb-5 border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          >
            ...
            <Text className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase">
              {i18n.language === 'hi' ? 'हिंदी (HI)' : 'اردو (UR)'}
            </Text>
          </TouchableOpacity>
        </View>
